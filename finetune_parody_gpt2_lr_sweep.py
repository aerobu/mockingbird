# Finetuning GPT2 on parody dataset with learning rate sweep
import os
import json
import torch
import random
...
lrs = [4e-5, 2e-5, 6e-5, 1e-4]
epochs = 1
batchSize = 4
gradAccum = 4
maxLen = 256
warmupFrac = 0.05
resultsDir = "results_lr_sweep"
...
def train_one_lr(lr_value):
    trialDir = os.path.join(resultsDir, f"lr_{lr_value}")
    ...
    model = GPT2LMHeadModel.from_pretrained(modelSrc)
    ...
    optim = AdamW(model.parameters(), lr=lr_value)
    steps_per_epoch = len(train_loader)
    total_steps = steps_per_epoch * epochs
    warmup_steps = int(total_steps * warmupFrac)
    train_losses = []
    dev_losses = []
    global_step = 0
    model.train()
